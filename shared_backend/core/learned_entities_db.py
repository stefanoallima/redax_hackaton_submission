...
- HF Spaces: /data/learned_entities_db.json (persistent storage)
- Desktop: ~/.oscuratesti/learned_entities.json (user home directory)
"""
...
class LearnedEntitiesDB:
    ...
    Environment-aware: Auto-detects HF Spaces vs Desktop deployment
    """

    @staticmethod
    def _get_default_path() -> Path:
        """
        Auto-detect environment and return appropriate default path

        Returns:
            - HF Spaces: /data/learned_entities_db.json
            - Desktop: ~/.oscuratesti/learned_entities.json
        """
        # Check if running on HF Spaces (has /data persistent storage)
        if os.path.exists('/data'):
            logger.info("Detected HF Spaces environment - using /data persistent storage")
            return Path('/data/learned_entities_db.json')
        else:
            logger.info("Detected desktop environment - using user home directory")
            return Path.home() / ".oscuratesti" / "learned_entities.json"
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        Args:
            db_path: Optional override path. If None, auto-detects environment.
        """
        self.db_path = db_path or self._get_default_path()
        self.db_path.parent.mkdir(parents=True, exist_ok=True)
        logger.info(f"Learned entities database path: {self.db_path}")
    ...
    def add_learned_entity(
        ...
        if normalized in self.data['entities']:
            ...
            self.data['metadata']['total_learned'] += 1
    ...
    def add_denied_entity(
        ...
        if normalized not in self.data['denied']:
            ...
            self.data['metadata']['total_denied'] += 1
