            ...
            for page_num in range(len(doc)):
                ...
                for entity in sorted_entities:
                    ...
                    if text_instances:
                        ...
                        for inst in text_instances:
                            ...
                            # Add redaction annotation - color depends on style
                            if redaction_style == 'solid_black':
                                page.add_redact_annot(inst, fill=(0, 0, 0))  # Black fill (traditional)
                            else:
                                page.add_redact_annot(inst, fill=(1, 1, 1))  # White fill (for placeholder)

                # Apply all redactions at once (removes original text)
                page.apply_redactions()

                # Add placeholder text only if using placeholder style
                if redaction_style == 'placeholder':
                    for replacement in replacements:
                        ...
                        fontsize = max(6, min(10, rect_height * 0.7))

                        # Use same rectangle (placeholder is same length as original)
                        ...
                        # Insert placeholder text with softer yellow background and bold black text
                        rc = page.insert_textbox(
                            display_rect,
                            placeholder,
                            fontsize=fontsize,
                            color=(0, 0, 0),  # Black text
                            fill=(1, 0.95, 0.6),  # Softer yellow background (was too bright)
                            align=fitz.TEXT_ALIGN_LEFT,  # Left align to preserve layout
                            fontname="helv",
                            render_mode=0  # Fill text (ensures black text is visible)
                        )

                        # If textbox insertion failed, try with smaller font
                        if rc < 0:
                            smaller_fontsize = max(5, fontsize * 0.8)
                            rc = page.insert_textbox(
                                display_rect,
                                placeholder,
                                fontsize=smaller_fontsize,
                                color=(0, 0, 0),
                                fill=(1, 0.95, 0.6),  # Same softer yellow
                            ...
                            if rc < 0:
                                page.draw_rect(display_rect, color=(1, 0.95, 0.6), fill=(1, 0.95, 0.6))
                                page.insert_text(
                                    (display_rect.x0 + 2, display_rect.y1 - 2),
                                    placeholder,
                                    fontsize=6,
                                    color=(0, 0, 0)
                                )
